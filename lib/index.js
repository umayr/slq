...
const safeSplitJoin = {
  obj: function (key, text, src) {
    var iter;
    var flag;
    var temp = text.split(' ' + key + ' ');

    for (iter = 0; iter < temp.length; iter++) { //find if word has 'key' in it
      if (temp[iter].indexOf(key) >= 0) {
        flag = true;
        break;
      }
    }
    text = (flag) ? text.split(' ' + key + ' ').join(' ' + src[key] + ' ') : text.split(key).join(src[key]);
    return text;
  }
};
...
const substitute = {
  'object': function handleObject(src, text) {
    ...
    keys.forEach(key => {
      text = text.split(/[key]+(?:^|\b)/g).join(src[key]);
    });
